...
King = new Class({
    ...
    ,create: co.wrap(function* create( resource, items, replace ){
        ...
        for( let idx=0, len=items.length; idx < len; idx++){
            ...
            resp = resp.statusCode === 409 ?
                yield this.request( replace ? 'put' : 'patch', resource, ( result.value.id || result.value.name ), result.value, [200, 201, 202, 204] ) :
                resp;
    ...
    ,update:co.wrap(function* update( resource, id, data){
        let resp = yield this.request('patch', resource, id, data, [200, 202, 204]);
        return {
            status:resp.statusCode,
            content:resp.body
        };
    ...
    ,_process_result:function _process_result( options, result, codes ) {
      ...
      // If no codes were specified then we consider specific 20* codes to be acceptable.
      if ((codes || [200, 201, 202, 204]).find(code => code === result.statusCode)) {
