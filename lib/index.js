...
King = new Class({
    ...
    ,create: co.wrap(function* create( resource, items, replace ){
        ...
        for( let idx=0, len=items.length; idx < len; idx++){
            ...
            resp = yield this.request('post', resource, null, result.value, [201, 409]);
            resp = resp.statusCode === 409 ? yield this.request( replace ? 'put' : 'patch', resource, ( result.value.id || result.value.name ), result.value, [200] ) : resp;
    ...
    ,destroy:co.wrap(function* destroy( resource, id ){
        let resp = yield this.request('delete', resource, id, undefined, [204, 404]);
        return resp.body;
    })
    ...
    ,list:co.wrap(function* list( resource, id ){
        let resp = yield this.request('get', resource, id, undefined, [200]);
    ...
    ,update:co.wrap(function* update( resource, id, data){
        let resp = yield this.request('patch', resource, id, data, [200, 201]);
        return {
            status:resp.statusCode,
            content:resp.body
        };
     ...
     **/
    ,request:function _request( action, resource, id, data, optionalAcceptableStatusCodes ){
        let requestOptions;

        //  Sometimes we send entire request options object as first parameter in which case
        //  we don't need to generate the request options.
        if (typeof action === 'object') {
            requestOptions = action;
            optionalAcceptableStatusCodes = resource;
        } else {
            let result = resource_exp.exec( resource )
        ...
        const acceptableStatusCodes = optionalAcceptableStatusCodes || [200, 201]

        return request(requestOptions).then((requestResult) => {
            if (acceptableStatusCodes.find(statusCode => statusCode === requestResult.statusCode)) {
                return requestResult;
            }

            debug('Kong API failed for', requestOptions.uri, 'with', requestResult.statusCode, requestResult.body);
            return Promise.reject(new Error(
                `Kong API failed with ${requestResult.statusCode}: ${JSON.stringify(requestResult.body)}`));
        });
    }
    ...
    ,plugins: co.wrap(function* plugins( id, data ){
        ...
        plugins_list = yield this.request({
            method:'GET'
          , uri:url
          ,json:true
        }, [200]);
